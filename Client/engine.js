	...
	var character = null;
		...
		loader.load('./Client/MODEL/ai5.json', function(geometry, materials){
			...
			// if (num == 0) {
				...
				// enemy = Player.make('124')
				//num = 1
			//}
			
			

			currentPlayer = Player.make(1)
			enemy = Player.make(2)
	...
	var Player = {
		make : function(id) {
			...
			var mesh = character.clone(false) // new THREE.Mesh(geometry, material);
	...
	// 	enemy.mesh.position.x += randx * 1
	// 	enemy.mesh.position.z += randz * 1
	// })
	...
	shoot= function() {
		if (currentPlayer != null) {
			...
			if (intersects.length > 0) {
				...
				// enemy.mesh.transparent = true
				// enemy.mesh.opacity = 0
				// console.log(123)

				arWorldRoot.remove(enemy.mesh)
